...
summary: "A priority-queue job scheduler with DAG workflows, retry backoff, a dead letter queue, and real-time updates, with no Celery and no external broker."
...
- A job scheduler that runs without Celery, RQ, or any managed queue: one Postgres-backed heap and one async worker pool, nothing else in the critical path.
- The database itself enforces claim deduplication, so a job can never run twice even under concurrency.
- Priority aging guarantees no job starves: every job is mathematically certain to reach high priority within a bounded window.
- Cascade retry re-runs a whole downstream DAG from a fixed root job with zero manual per-step re-triggers.
